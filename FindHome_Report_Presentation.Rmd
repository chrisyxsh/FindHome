...
```{r eval=FALSE,echo=FALSE,message=FALSE,error=FALSE,warning=FALSE,results='hide'}
...
```
```{r eval=TRUE,echo=FALSE,message=FALSE,error=FALSE,warning=FALSE,fig.fullwidth=TRUE, fig.width=9, fig.height=6}
...
pub_msa_school_map
```
Here we use different colors for different areas according the highest rank of schools in each area because a common thinking: Which one is the best school in this area ? 
Clicking on a area you can view a list of public schools in this area ordered by school's rank.

***

```{r eval=TRUE,echo=FALSE,message=FALSE,error=FALSE,warning=FALSE,results='hide',fig.fullheight=TRUE, fig.width=10, fig.height=6}
...
```{r eval=TRUE,echo=FALSE,message=FALSE,error=FALSE,warning=FALSE,results='hide',fig.fullwidth=TRUE, fig.width=11, fig.height=6}
...
Here we use 22 distinctive colors to mark each occupation's employment by it's occupation category.
A_Seattle and A_Portland obviously have more employment.
...
```{r eval=TRUE,echo=FALSE,message=FALSE,error=FALSE,warning=FALSE,results='hide',fig.fullheight=TRUE, fig.width=11, fig.height=6}
...
```{r eval=TRUE,echo=FALSE,message=FALSE,error=FALSE,warning=FALSE,results='hide',fig.fullwidth=TRUE, fig.width=12, fig.height=7}
...
```{r eval=TRUE,echo=FALSE,message=FALSE,error=FALSE,warning=FALSE,results='asis',out.width = '50%'}
library(knitr)
...
Why they did this? By this combine,can people really work and live as similar as in A_Seattle? To answer those questions, it seems that I need doing Data_Word_Science.
...
ggplot(Seattle_VanPort_08_17_stat, aes(x=Year,y=SOC.CategoryName,fill=employ_sum)) + 
  ...
  scale_y_discrete("Occypation Category") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  guides(fill = guide_legend(ncol=2,label.position = "left", 
                             label.hjust = 1,
                             keyheight = 1,
...
Here we put mean, min and max values together for the past five years.
